...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector)
{
    ...
    if(diskstreamer_seek(stream, directory_start_pos) != kYOKOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    while(1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != kYOKOS_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory)
{
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != kYOKOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != kYOKOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_resolve(struct disk* disk)
{
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != kYOKOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != kYOKOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
void fat16_get_fulle_relative_filename(struct fat_directory_item* item, char*out, int max_len)
{
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name)
{
    ...
    char tmp_filename[KYOKOS_MAX_PATH];
    for(int i = 0; i<directory->total; i++)
    {
        fat16_get_fulle_relative_filename(&directory->item[i], tmp_filename, sizeof(tmp_filename));
